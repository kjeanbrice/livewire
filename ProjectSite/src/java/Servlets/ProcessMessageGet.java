 ...
 * @author peterjasko
 */
@WebServlet(name = "ProcessMessageGet", urlPatterns = {"/ProcessMessageGet"})
public class ProcessMessageGet extends HttpServlet {
    
    public ProcessMessageGet(){
        super();
    }
     protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(userId == 0){
            ...
            try{
                ...
                messages = DatabaseUtils.get_messages(connection, userId);
                String returnString = "{\"messages\":[ ";
                for(int i = 0; i < messages.size();i++) {
                    System.out.println(messages.get(i));
                   returnString += ((MessageData)(messages.get(i))).generateJSON();
                   returnString += ",";
                }
